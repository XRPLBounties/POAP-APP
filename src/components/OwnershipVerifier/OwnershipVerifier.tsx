...
type Actions = {
  ...
  reset: () => void;
  setOwnerWalletAddress: (value?: string) => void;
};
...
const useStore = create<State & Actions>()((set, get) => ({
  ...
  setOwnerWalletAddress: (value?: string) => {
    set({ ownerWalletAddress: value });
  },
}));
...
function OwnershipVerifier() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (mounted) {
          setData(event || null);
          state.setOwnerWalletAddress(event?.ownerWalletAddress);
        }
      } catch (err) {
        console.debug(err);
        ...
        if (mounted) {
          setData(undefined);
          state.setOwnerWalletAddress(undefined);
        }
  ...
  const createStepFields = (state: State & Actions, step: Steps) => ({
    ...
    ownerWalletAddress: state.ownerWalletAddress,
  ...
  return (
    <Box>
      ...
      >
        {[Steps.RESULT].includes(state.activeStep) && (
        ...
        {data && (
          <React.Fragment>
            ...
            <ResultStep {...createStepFields(state, Steps.RESULT)} />
          </React.Fragment>
        )}
      </Box>
      <Stepper
